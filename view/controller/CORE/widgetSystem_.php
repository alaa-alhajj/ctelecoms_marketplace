...
class widgets extends utils {
	...
	function getFilters($settings_id){
		...
		//$query = $this->fpdo->from('cms_filter_fields')->where(" settings_id='$settings_id' ");
		//echo $query->getQuery();
	...
	function getWidget($widget_id,$page_id,$condition){
		...
		if($wid_html){
			...
			$final_condition = stripcslashes($final_condition);
			if($lang_type=='Table'){if($final_condition!=''){$final_condition.=' AND ';}$final_condition.=" lang='".$_SESSION['pLang']."' ";}
			...
			foreach ($query as $row) {
				...
				foreach($query2 as $row2){
					...
					if($module_field_lang_eff==1){
						$item_val = $item_val2 =$row[$module_field_title.'_'.$_SESSION['pLang']];
					}else{
						$item_val = $item_val2 =$row[$module_field_title];
					}
					/********************************************************************************/
			...
			if($wid_settings['pagination_id']){
				$final_html .= $this->getPagination($wid_settings['pagination_id'],$widget_total_numrows,$wid_settings['limit_num'],$link);
			}
		}
