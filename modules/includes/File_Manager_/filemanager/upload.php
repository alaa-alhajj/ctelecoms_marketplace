...
if (!empty($_FILES)) {
    ...
    if (in_array(fix_strtolower($info['extension']), $ext)) {
        ...
        $chrs = array( '&', '$', '_', ' ');
       echo $targetFile = strtolower(str_replace($chrs, "_", $targetPath . $_FILES['file']['name']));
...
$chrs = array('/', '&', '$', '_', ' ');
        ...
        // upload
        $slang = "SELECT * FROM `languages` WHERE default_lang=1 and type='view'";
        $test = 0;
        $reslang = mysql_query($slang);
        while ($r = mysql_fetch_array($reslang, MYSQL_ASSOC)) {
            $ActiveLang = $r['lang'];
        }
        $file0 = strtolower(str_replace($chrs, "_", $info['filename']));
        $file1 = strtolower(str_replace($chrs, "_", $_FILES['file']['name']));
        $fileExt = strtolower($info['extension']);
        $siz = $_FILES['file']['size'];
        $t = str_replace('../', '', $targetPath);
        $targetPath = $t;
        $query = "INSERT INTO files (folder,file,name,name_ar,name_en,ext,size)values('$targetPath','$file1','$file0','$file0','$file0','$fileExt','$siz')";
