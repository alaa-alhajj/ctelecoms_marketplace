...
$json_array = $_SESSION['json_selected'];

$json_array = stripcslashes($json_array);
$json_array = str_replace('""', '', $json_array);
$json_array = str_replace('"[', '[[', $json_array);
$json_array = str_replace(']"', ']]', $json_array);
$json_array = str_replace('][', '],[', $json_array);
$json_array = substr($json_array, 1, strlen($json_array));
...
//filter
    ...
    ?>
                onSave: function(imageID, newURL) {alert('onsave');
                        show_animation();
        ...
        <body>
            ...
            <input type="hidden" id="ok" value="<?php echo trans('OKay'); ?>" />
            <input type="hidden" id="cancel" value="<?php echo trans('Cancel'); ?>" />
            <input type="hidden" id="rename" value="<?php echo trans('Renaming'); ?>" />
                    ...
                    <div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
                        <?php if ($java_upload) { ?>
                            ...
                            <div class="tab-content">
                                ...
                                <?php } ?>
                                <form action="dialog.php" method="post" accept-charset="utf-8" enctype="multipart/form-data" id="rfmDropzone" class="dropzone">
                                    ...
                                    <div class="fallback">
                                        ...
                                        <input type="submit" name="submit" value="<?php echo trans('OKay') ?>" />
                                </form>
                            </div>
...
// Should lazy loading be enabled
            ...
            foreach ($files as $file_array) {
                $d = date('Y-m-d h:i:s', ($file_array['date']));
                $f8 = $file_array['file'];
                $siz = $file_array['size'];

                $tpath = str_replace('../', '', $current_path . $rfm_subfolder . $subdir);
                $ssql = "select * from `files` where `file`='$f8' and folder='$tpath'";
                $results = mysql_query($ssql);
                $ni = mysql_num_rows($results);

                if ($ni == 0 && $f8 != '.' && $f8 != '..' && $file_array['extension'] != 'dir' && $siz != '') {

                    $ext8 = $file_array['extension'];
                    $si = $file_array['size'];
                    $pth = $tpath;
                    $ff = preg_replace("/\.[^.]+$/", "", $f8);
                    $sq = "INSERT INTO `files`( `folder`, `file`, `name`, `name_en`, `name_ar`, `ext`,`size`) VALUES ('$pth','$f8','$ff','$ff','$ff','$ext8','$si');";
                    $results1 = mysql_query($sq);
                }
            }
            //====================================================================================================================================================
            //====================================================================================================================================================
...
//                                $f = $file_array['file'];
//                                $siz = $file_array['size'];
//                                        $ssql="select * from `files` where `file`='$f'";
//                                $results=mysql_query($ssql);
//                                $ni=mysql_num_rows($results);
//                                if ($ni==0 && $f != '.' && $f != '..' && $file_array['extension']!='dir' && $siz != '' ){
//                                                    
//                                                    $ext = $file_array['extension'];
//                                                    $si = $file_array['size'];
//                                                    $sq = "INSERT INTO `files`( `folder`, `file`, `name`, `name_en`, `name_ar`, `ext`,`size`) VALUES ('$subdir','$f','$f','$f','$f','$ext','$si')";
//                                                    mysql_query($sq);
//                                                       file_put_contents('67', $sq);
//                                                }
                                //here
                                //------------------------------------------------------------------------

                                if ($d != '' && $f != '' && $f != 'cash' && $siz != '') {
                                    $Query_id = "SELECT max(id) as voila_id FROM `files` WHERE  `file`='$f'";

                                    $res = mysql_query($Query_id);
                                    if (mysql_num_rows($res) > 0) {
                                        $voila_id = mysql_result($res, 0, 'voila_id');
                                        $file_array['voila_id'] = $voila_id;
                                    }
                                }
                                //------------------------------------------------------------------------
                                ...
                                elseif ($file != '..') {
                                    ...
                                    if ($file == '..')
                                        ...
                                        echo 'dir';
                                    ?>" <?php if (($filter != '' && stripos($file, $filter) === false)) echo ' style="display:none;"'; ?>><?php
                                        ...
                                        ?>">
                                            ...
                                            <a class="folder-link" href="dialog.php?<?php echo $get_params . rawurlencode($src) . "&" . uniqid() ?>">
                                                <div class="img-precontainer">
                                                    <div class="img-container directory"><span></span>
                                                        <img class="directory-img"  src="img/<?php echo $icon_theme; ?>/folder<?php
                                                        if ($file == "..") {
                                                            echo "_back";
                                                        }
                                                        ?>.png" />
                                            ...
                                            <?php } else { ?>
                                                ...
                                                <div   class="box" id="clicked<?php
                                                $s = str_replace('.', '', $file);
                                                echo str_replace(' ', '-', $s);
                                                ?>" >
                                                    <h4 class="<?php
                                                    if ($ellipsis_title_after_first_row) {
                                                        echo "ellipsis";
                                                    }
                                                ...
                                                <input type="hidden" class="voila_id" value="<?php echo $voila_id; ?>"/>
                            ...
                            foreach ($files as $nu => $file_array) {
                                ...
                                if ($file == '.' || $file == '..' || is_dir($current_path . $rfm_subfolder . $subdir . $file) ||
                                        in_array($file, $hidden_files) ||
...
//                                                    $ext = $file_array['extension'];
//                                                    $si = $file_array['size'];
//                                                  //  $sq = "INSERT INTO `files`( `folder`, `file`, `name`, `name_en`, `name_ar`, `ext`,`size`) VALUES ('$current_path','$f','$f','$f','$f','$ext','$si')";
...
//                                                }
                                    ...
                                    <li class="ff-item-type-<?php echo $class_ext; ?> file"  data-name="<?php echo $file; ?>" <?php if (($filter != '' && stripos($file, $filter) === false)) echo ' style="display:none;"'; ?>><?php
                                        ...
                                        ?>">
                                            ...
                                            if ($d != '' && $f != '' && $f != 'cash' && $siz != '') {
                                                ...
                                                $Query_id = "SELECT max(id) as voila_id,name_en as name FROM `files` WHERE   `file`='$f'";

                                                $res = mysql_query($Query_id);
                                                $check = array();
                                                if (mysql_num_rows($res) > 0) {
                                                    $voila_id = mysql_result($res, 0, 'voila_id');
                                                    $voila_name = mysql_result($res, 0, 'name');
                                                    $voila_file = mysql_result($res, 0, 'file');
                                                    $file_array['voila_id'] = $voila_id;
                                                    ...
                                                    foreach ($json_array as $key => $val) {
                                                        ...
                                                        foreach ($val as $ke => $v) {
                                                            ...
                                                            if ($v == $voila_id) {
                                                                $check = array_push($check, $voila_id);
                                                            }
                                                        }
                                                    }
                                                }
                                            ...
                                            ?>
                                            <a href="javascript:void('')" class="link" data-file="<?php if($_GET['get_id']=='1') {echo $voila_id;}else{echo $file;} ?>" data-function="<?php echo $apply; ?>">
                                                <div class="img-precontainer">
                                                ...
                                                <div class="img-precontainer-mini <?php if ($is_img) echo 'original-thumb' ?>">
                                                    <div class="filetype <?php echo $extension_lower ?> <?php if (in_array($extension_lower, $editable_text_file_exts)) echo 'edit-text-file-allowed' ?> <?php
                                                    if (!$is_icon_thumb) {
                                                        echo "hide";
                                                    }
                                            ...
                                            <div  <?php if (in_array($voila_id, $check) !== false) { ?>  style="background-color:red;color:black !important;"
                                                                                                      <?php } else  ?> style="background-color:white;" id="clicked<?php
                                                                                                      $s = str_replace('.', '', $file);
                                                                                                      $s = str_replace(')', '', $s);
                                                                                                      $s = str_replace('(', '', $s);
                                                                                                      echo str_replace(' ', '', $s);
                                                                                                      ?>"  class="box" rel="<?php echo $voila_id; ?>">
                                                <h4 class="<?php
                                                if ($ellipsis_title_after_first_row) {
                                                    echo "ellipsis";
                                                }
                                                ?>"><a  <?php if (in_array($voila_id, $check) !== false) { ?>  style="color:black !important;"
                                                                                                         <?php } else { ?> style="color:black;"  <?php } ?> href="javascript:void('')" class="link" data-file="<?php echo $file; ?>" data-function="<?php echo $apply; ?>">
                                                        <?php echo $voila_name ?></a> </h4>
                                            </div>
                                            <input type="hidden" class="voila_id" value="<?php echo $file_array['voila_id']; ?>"/>
                                            <input type="hidden" class="date" value="<?php echo $file_array['date']; ?>"/>
                                            <input type="hidden" class="size" value="<?php echo $file_array['size'] ?>"/>
                                            ...
                                            <figcaption>
                                                <form accept-charset="utf-8" action="force_download.php" method="post" class="download-form" id="form<?php echo $nu; ?>">
                                                    ...
                                                    <?php } ?>
                                                    <a 
                                                        onclick="edit(<?php echo $voila_id ?>, '<?php echo $file_path ?>',
                                                                            '<?php echo fix_filename($filename, $transliteration) ?>',
                                                                            '<?php echo $transliteration ?>')" id="renaming" rel="<?php echo $voila_id ?>" data-toggle="modal" href="#form-content" class="tip-left edit-button rename-file-paths <?php
                                                        if ($rename_files && !$file_prevent_rename)
                                                            echo "rename-file1";
                                                        ?>" title="<?php echo trans('Rename') ?>" 
                                                        data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>" data-thumb="<?php echo $thumbs_path . $subdir . $file; ?>">
    ...
    <div class="container" style="width:100% !important">
        <div class="well well-large">
            ...
            <div id="form-content" class="modal hide fade in" style="display: none;">
                ...
                <form id="EditForma" accept-charset="utf-8">
                    <div id="infoEdit" class="modal-body">

                    </div>
                </form>
                <div class="modal-footer">
                    <input class="btn btn-success" id="infoEditOk" type="submit" value="Edit" >
                    <a href="#" class="btn" id="infoEditCancel" data-dismiss="modal">Cancel</a>
                </div>
            </div>
    <!--            <div id="thanks"><p><a data-toggle="modal" href="#form-content" class="btn btn-primary btn-large">Edit button</a></p></div>-->
        </div>
    </div>
    ...
    <script>
                function colorThis(id){
                //             alert('clicked');
                //             $('.box[rel='+id+']').css({'background-color': 'red'});
                //         var out = {
                //                output: []
                //            };      
                //        var va = $('#fieldID3').val();
                ...
                //                        $('[rel=' + result['id'] + ']').addClass('red');
                //                        $('[rel=' + result['id'] + ']').css({'background-color': 'red'});
                //
                //                    });
                //
                //
        ...
        function edit(id, old_path, old_name, t){
        console.log(old_path);
                ...
                type: "POST",
                        url: "process.php", // 
                        data: {id:id, fildName:'<?php echo $field_id ?>'},
                        success: function(msg){

                        $('#infoEdit').html(msg);
                                $('#infoEditOk').click(function (){
                        var form = document.querySelector('form');
                                var data = new FormData(form);
                                // var req = new XMLHttpRequest();
                                // req.send(data);
                                console.log(data);
                                console.log($('#EditForma').serialize());
                                $.ajax({
                                type:"post",
                                        url:"edit.php",
                                        data:$('#EditForma').serialize() + '&old_path=' + old_path + '&old_name=' + old_name + '&t=' + t,
