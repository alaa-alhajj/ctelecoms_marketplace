        ...
        self.each(function(baseIndex, baseElem) {
            ...
            function createColControls() {
                canvas.find('.column').each(function() {
                    ...
                    createTool(drawer, 'Add Image', 'ge-add-image', 'glyphicon-picture', function() {

                        $(this).attr("href", _PREF + _MODULES_FOLDER + "/includes/File_Manager/filemanager/dialog.php?type=0&field_id=" + fie + "&relative_url=1&get_id=1");
            ...
            function FilterClass(cssClass) {

                var getCss = cssClass;
                for (var i = 0; i < colClasses.length; i++) {
                    for (var j = 12; j >= 1; j--) {
                        var cCol = j;
                        getCss = getCss.replace(colClasses[i] + "" + cCol + " ", "");
                    }
                }
                getCss = getCss.replace("ui-sortable", "");
                getCss = getCss.replace("column", "");
                getCss = getCss.replace("row", "");
                return getCss;
            }
            function ReturnColsClass(cssClass) {

                var getCss = [];
                var ArrayCss = cssClass.split(" ");

                for (var i = 0; i < ArrayCss.length; i++) {
                    if (ArrayCss[i].indexOf("col-sm-") > -1 || ArrayCss[i].indexOf("col-md-") > -1 || ArrayCss[i].indexOf("col-xs-") > -1) {
                        getCss.push(ArrayCss[i]);
                    }
                    if (ArrayCss[i].indexOf("ui-sortable") > -1) {
                        getCss.push(ArrayCss[i]);
                    }
                    if (ArrayCss[i] === "column") {
                        getCss.push(ArrayCss[i]);
                    }
                    if (ArrayCss[i] === "row") {
                        getCss.push(ArrayCss[i]);
            ...
            function createDetails(container, cssClasses) {
                var orginClass = container.attr('class');
                var detailsDiv = $('<div class="ge-details" />');

                $('<input class="form-control ge-ide" />')
                        .attr('placeholder', 'ID')
                ...
                $('<input class="form-control ge-class" />')
                        .attr('placeholder', 'CSS Class')
                        .val(FilterClass(container.attr('class')))
                        .attr('title', 'Set Classes')
                        .appendTo(detailsDiv)
                        ;

                $("<button class='btn btn-default'>Save</button>").on("click", function() {
                    container.attr('id',  $(this).prev(".ge-class").prev(".ge-ide").val());
                    container.attr('class', ReturnColsClass(orginClass) + " " + $(this).prev(".ge-class").val()); 
                    console.log(container.children().find(".ge-details"));
                    container.children().find(".ge-details").hide();
                }).appendTo(detailsDiv);
        ...
        init: function(settings, contentAreas) {
            ...
            contentAreas.each(function() {
                ...
                if (!contentArea.hasClass('active')) {
                    ...
                    contentArea.addClass('active');
                    console.log(settings.tinymce);
                    var configuration = $.extend(
                            (settings.tinymce && settings.tinymce.config ? settings.tinymce.config : _tinyMceOptions),
                            {
                                inline: true,
                                oninit: function(editor) {
                                    try {
