...
if (isset($_REQUEST) && $_REQUEST['action'] == 'Edit') {
    $save_ob = new saveform($db_table, $_REQUEST, $colsPhoto, 'id');
    if ($_REQUEST['saveClose'] != "") {
        $utils->redirect($pageList);
    } else {
        $utils->redirect($pageInsertFeature . "?cat_id=" . $_REQUEST['id']);
    }
}
echo $path = '<ul id="breadcrumbs-one">
    <li><a href="#">Category Info</a></li>
     <li class="active-menue"><a href="#">Category Photos</a></li>
...
$form->setSkipBtn(true, $pageInsertFeature . "?id=" . $_REQUEST['id']);
