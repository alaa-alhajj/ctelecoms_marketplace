...
class LiveZillaAPI
{
	...
	{
		return Encoding::Base64UrlEncode($_value);
	}
	...
	{
		DBManager::Execute(true,"DELETE FROM `".DB_PREFIX.DATABASE_OPERATORS."` WHERE `id`='".DBManager::RealEscape($_loginId)."' LIMIT 1;");
		DBManager::Execute(true,"DELETE FROM `".DB_PREFIX.DATABASE_PREDEFINED."` WHERE `internal_id`='".DBManager::RealEscape($_groupId)."';");
	}
	...
	{
		DBManager::Execute(true,"DELETE FROM `".DB_PREFIX.DATABASE_GROUPS."` WHERE `id`='".DBManager::RealEscape($_groupId)."' LIMIT 1;");
		DBManager::Execute(true,"DELETE FROM `".DB_PREFIX.DATABASE_PREDEFINED."` WHERE `group_id`='".DBManager::RealEscape($_groupId)."';");
	}
	...
	{
		$result = DBManager::Execute(true,"SELECT MAX(`id`) AS `pcount` FROM `".DB_PREFIX.DATABASE_PREDEFINED."`;");
	...
	public function GetChatBillingTypes()
	{
		...
		return Server::$Configuration->Database["cct"];
	}
}
?>