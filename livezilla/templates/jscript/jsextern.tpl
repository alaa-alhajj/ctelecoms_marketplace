...
function lz_chat_set_groups(_chatPossible, _groups, _errors, _selected)
{
    try
    {
        ...
        if(_errors.length > 0)
        {
            ...
            lz_chat_show_setup_error(_errors);
...
{
    lz_chat_get_frame_object('lz_chat_loading').style.display = "block";
...
function lz_check_missing_inputs(_display,_alert,_contFunc)
{
	...
	for(var i = 0;i < lz_chat_data.InputFieldIndices.length;i++)
	{
		...
		else if(lz_chat_data.InputFieldValues[i].Active || (findex == 116 && lz_chat_data.CallMeBackMode))
		{
            ...
            if((((findex == 115 && lz_chat_data.SelectedGroup.ChatVouchersRequired.length > 0 && lz_chat_data.CurrentApplication=="chat") || (lz_array_indexOf(ihidden,findex) == -1 && (lz_array_indexOf(imandatory,findex) != -1 || lz_chat_data.InputFieldValues[i].Validation))) || (findex == 116 && lz_chat_data.CallMeBackMode) || (findex == 111 && lz_chat_data.IsPublicGroupChat)) && !isFilled)
			{
				if(_display)
                {
