...
if(isset($_POST[POST_EXTERN_SERVER_ACTION]))
{
    ...
    define("IS_FILTERED",Server::$Filters->MatchUser(Communication::GetIP(),LocalizationManager::ImplodeLanguages(((!empty($_SERVER["HTTP_ACCEPT_LANGUAGE"])) ? $_SERVER["HTTP_ACCEPT_LANGUAGE"] : "")),Encoding::Base64UrlDecode($_POST[POST_EXTERN_USER_USERID]),$externalUser->GeoCountryISO2));
    ...
    $error=buildLoginErrorField($externalUser,$externalUser->Browsers[0]);
    ...
    else
    {
        if($_POST[POST_EXTERN_SERVER_ACTION] == "search_kb")
        {
            ...
            $openExternal = !empty(Server::$Configuration->File["gl_kbin"]) && !$kbonly;
            if($query == "%ALL%")
            {
                ...
                if(!$openExternal && !empty($result) && !empty($main))
                    $externalUser->AddFunctionCall("lz_chat_search_result(false,'".base64_encode(Server::Replace($main,true,false,false,false))."',0);",false);
                else if(count($matches)==0 || isset($_GET["e"]))
                    $externalUser->AddFunctionCall("lz_chat_search_result(false,'".base64_encode("<div><br><br>".LocalizationManager::$TranslationStrings["client_kb_no_entries"]."</div>" . $result)."',0);",false);
                else{
                    $externalUser->AddFunctionCall("lz_chat_search_result(true,'".base64_encode("")."',0);",false);
                }
            }
            else if(strlen($query)>=Server::$Configuration->File["gl_kbml"])
            {
                $matches = KnowledgeBase::GetMatches($root,$query,Visitor::$BrowserLanguage);
                KnowledgeBase::ReqisterQuery($query,$externalUser->Browsers[0]);
                if(count($matches)>0)
                {
                    foreach($matches as $match)
                    ...
                    $res = IOStruct::GetFile(PATH_TEMPLATES . "kb_result_category.tpl");
                    $res = str_replace("<!--title-->","\"".Str::Cut($query,50,true)."\"",$res);
                    $res = str_replace("<!--entries-->",$result,$res);
                    $res = str_replace("<!--search-->","true",$res);
                    $res = str_replace("<!--id-->","sr",$res);
                    $externalUser->AddFunctionCall("lz_chat_search_result(false,'".base64_encode("<div><b>".str_replace("<!--count-->",count($matches),LocalizationManager::$TranslationStrings["client_kb_results_found"])." </b><br><br>" . $res)."',".count($matches).");",false);
                }
                else
                    $externalUser->AddFunctionCall("lz_chat_search_result(false,'".base64_encode("<div><br><br>".LocalizationManager::$TranslationStrings["client_search_no_result"]."</div>" . $result . "")."',0);",false);
            }
            else
                $externalUser->AddFunctionCall("lz_chat_search_result(false,'".base64_encode("<div><br><br>".LocalizationManager::$TranslationStrings["client_search_no_result"]."</div>" . $result . "")."',0);",false);
