        ...
        else
        {
            ...
            if(!empty($_POST["p_created_after"]))
            {
                if(is_numeric($_POST["p_created_after"]) && !empty($_POST["p_created_after"]))
                    $sql_where .= " AND `t2`.`time` > ". $_POST["p_created_after"];
                else
                {
            ...
            if(!empty($_POST["p_created_before"]))
            {
                if(is_numeric($_POST["p_created_before"]) && !empty($_POST["p_created_before"]))
                    $sql_where .= " AND `t2`.`time` < ". $_POST["p_created_before"];
                else
                {
    ...
    static function GetObjectFields($_objName)
    {
        ...
        if($_objName=="Ticket")
            return array(
                ...
                "Channel"=>array("Input"=>true,"Output"=>true,"Type"=>"int","Description"=>"Channel<br><br>[0] = Web<br>[1] = Email<br>[2] = Phone<br>[3] = Misc<br>[4] = Chat<br>[5] = Rating","Required"=>false,"Example"=>"0, <i>added in LiveZilla 6.1.0.0</i>; replaces <i>CreationType</i> available in previous versions.","MultilineDescription"=>true),
                "SubChannel"=>array("Input"=>true,"Output"=>true,"Type"=>"string","Description"=>"Sub-Channel Name","Required"=>false,"Example"=>"Sub Channel Name, <i>added in LiveZilla 6.1.0.0</i>","MultilineDescription"=>false),
