...
class OverlayChat
{
    ...
    function GetDeclinedMessage()
    {
        ...
        $dmsg .= " <a href=\"javascript:lz_chat_require_leave_message();\"><b>" . LocalizationManager::$TranslationStrings["client_leave_a_message"] . "</b></a>";
    ...
    function Init()
    {
        ...
        {
            $USER->Browsers[0]->SetTargetOperatorId(VisitorChat::$Router,$USER,$this->Botmode,$this->Botmode,null,true,$USER->Browsers[0]->Forward != null,$this->Botmode);

            //$USER->Browsers[0]->FindOperator(VisitorChat::$Router,$USER,$this->Botmode,$this->Botmode,null,true,$USER->Browsers[0]->Forward != null);
    ...
    function GetPostHTML($_text,$_translation,$_add,$_operator,$_name,$_time,$_senderId)
    {
        ...
        $post = ($_add) ? ((!$_operator) ? $exchtmpl : IOStruct::GetFile(TEMPLATE_HTML_MESSAGE_OVERLAY_CHAT_OPERATOR)) : ((!$_operator) ? IOStruct::GetFile(TEMPLATE_HTML_MESSAGE_OVERLAY_CHAT_ADD) : IOStruct::GetFile(TEMPLATE_HTML_MESSAGE_OVERLAY_CHAT_OPERATOR_ADD));
    ...
    static function ReplaceColors($_html,$_operator)
    {
        ...
        $_html = str_replace("<!--bgcd-->",Colors::TransformBrightness($primary,-0.2),$_html);
