...
class Logging
{
    static function SecurityLog($_type,$_value="",$_user="")
    {
        if(Is::Defined("DB_CONNECTION"))
        {
            ...
            if(!DEBUG_MODE)
                Str::Cut($request,1024);
            else
                Str::Cut($request,3000);

            DBManager::Execute(true,"INSERT INTO `".DB_PREFIX.DATABASE_ADMINISTRATION_LOG."` (`id`,`type`,`value`,`trace`,`time`,`user`) VALUES ('".DBManager::RealEscape(getId(32))."','".DBManager::RealEscape($_type)."','".DBManager::RealEscape($_value)."','".DBManager::RealEscape($request)."','".DBManager::RealEscape(time())."','".DBManager::RealEscape($_user . " " . Communication::GetIP(true))."');");
...
class SystemTime
{
    ...
    static function GetExecutionTime($_start=true)
    {
        if($_start)
            ...
            return Str::Cut(microtime(true) - SystemTime::$StartTime,8);
    }
}
...
class Encoding
{
    ...
    {
        return str_replace(array('=','+','/'),array('_','-',','),$_input);
    }
...
class Server
{
    ...
    {
        if(!empty(CacheManager::$ActiveManager) && CacheManager::$ActiveManager->GetData(DATA_CACHE_KEY_FILTERS,Server::$Filters))
            return;
...
class FilterList
{
    ...
    function MatchEmail($_email,$_subject)
    {
        ...
        foreach($this->Filters as $filter)
        {
            ...
            if($match)
                Logging::EmailLog("Blocked Email by filter '". $filter->Filtername . "' - " .  (($compare["match_email"]) ? "Email match " : "Subject match ") . $_email . " - " . $_subject);
...
class Ticket extends Action
{
	...
	function GetXML($_full,$_demand=false)
	{
		if($_full)
		{
			$xml = "<val id=\"".base64_encode($this->Id)."\" u=\"".base64_encode($this->LastUpdated)."\" w=\"".base64_encode($this->WaitBegin)."\" del=\"".base64_encode($this->Deleted ? "1" : "0")."\" wl=\"".base64_encode($this->WatchList ? "1" : "0")."\" gr=\"".base64_encode($this->Group)."\" l=\"".base64_encode($this->Language)."\" h=\"".base64_encode($this->GetHash())."\" p=\"".base64_encode($this->Priority)."\" t=\"".base64_encode($this->Channel)."\" s=\"".base64_encode($this->SubChannel)."\">\r\n";
    ...
    function SendEditorReply($_messageId, $_email, $_qMessageId="")
    {
        ...
        for($i=0;$i<count($tsData);$i++)
        {
            ...
            $tText = (($i==0) ? $tText : nl2br($tText)).$lb.$lb;
            ...
            $tsData[$i] = Mailbox::FinalizeEmail($tsData[$i],$i==1,$this->Channel >= 6);
        }
	...
	function Save($_hash="",$_saveMessages=true)
	{
		...
		if(DBManager::Execute(true,"INSERT IGNORE INTO `".DB_PREFIX.DATABASE_TICKETS."` (`id`,`user_id`,`target_group_id`,`hash`,`channel_type`,`sub_channel`,`iso_language`,`channel_id`,`channel_conversation_id`) VALUES ('".DBManager::RealEscape($this->Id)."', '".DBManager::RealEscape($this->SenderUserId)."', '".DBManager::RealEscape($this->Group)."', '".DBManager::RealEscape($_hash)."', '".DBManager::RealEscape($this->Channel)."', '".DBManager::RealEscape($this->SubChannel)."', '".DBManager::RealEscape($this->Language)."', '".DBManager::RealEscape($this->ChannelId)."', '".DBManager::RealEscape($this->ChannelConversationId)."');"))
        {
    ...
    {
        if($_user)
            return "`user_id` LIKE '%".$_sFor."%'";
        return "LOWER(`".DB_PREFIX.DATABASE_TICKETS."`.`sub_channel`) LIKE '%".$_sFor."%' OR LOWER(`".DB_PREFIX.DATABASE_TICKETS."`.`hash`) LIKE '%".$_sFor."%' OR LOWER(`te`.`sub_status`) LIKE '%".$_sFor."%' OR LOWER(`do`.`fullname`) LIKE '%".$_sFor."%' OR `tm`.`sender_id` LIKE '%".$_sFor."%' OR `tm`.`ticket_id` LIKE '%".$_sFor."%' OR LOWER(`tc`.`value`) LIKE '%".$_sFor."%' OR LOWER(`tm`.`text`) LIKE '%".$_sFor."%' OR LOWER(`tm`.`fullname`) LIKE '%".$_sFor."%' OR LOWER(`tm`.`email`) LIKE '%".$_sFor."%' OR LOWER(`tm`.`company`) LIKE '%".$_sFor."%' OR LOWER(`tm`.`phone`) LIKE '%".$_sFor."%' OR LOWER(`tm`.`subject`) LIKE '%".$_sFor."%'";
    }

    static function GetMatchCount($_sFor)
    {
        $result = DBManager::Execute(true,"SELECT COUNT(*) as `mcount` FROM `".DB_PREFIX.DATABASE_TICKETS."` WHERE ". Ticket::GetSearchQuery($_sFor,true));
        if($result && $row = DBManager::FetchArray($result))
            return $row["mcount"];
        return 0;
    }
...
class WebsitePush extends Action
{
	...
	{
		return "lz_tracking_init_website_push('".base64_encode(str_replace("%target_url%",VisitorMonitoring::Replace($this->TargetURL),$this->Text))."',".time().");";
	}
	
	function GetExecCommand()
	{
		return "lz_tracking_exec_website_push('".base64_encode(VisitorMonitoring::Replace($this->TargetURL))."');";
	}
...
class OverlayBox extends Action
{
	...
	function Save()
	{
		DBManager::Execute(true,$d="INSERT INTO `".DB_PREFIX.DATABASE_OVERLAY_BOXES."` (`id`, `created`, `receiver_user_id`,`receiver_browser_id`,`event_action_id`, `url`,`content`, `displayed`, `closed`) VALUES ('".DBManager::RealEscape($this->Id)."', '".DBManager::RealEscape(time())."','".DBManager::RealEscape($this->ReceiverUserId)."', '".DBManager::RealEscape($this->BrowserId)."','".DBManager::RealEscape($this->EventActionId)."','".DBManager::RealEscape($this->URL)."','".DBManager::RealEscape($this->Text)."',0,0);");
    }
	
	function CreateOverlayTemplate($_style,$_siteName,$_cwWidth,$_cwHeight,$_serverURL,$_clSpace=46)
	{
		...
		$template = str_replace("<!--width-->",$_cwWidth-$_clSpace,$template);
...
class Event extends BaseObject
{
    ...
    function MatchDataConditions($_browser)
    {
        ...
        foreach($this->DataConditions as $dcarr)
        {
            ...
            foreach(Server::$Inputs as $input)
            {
                ...
                {
                    if(Is::WildcardMatch($value,$input->GetValue($_browser)))
                        return true;
                }
            }
        }
	...
	function GetSQL()
	{
		return "INSERT INTO `".DB_PREFIX.DATABASE_EVENTS."` (`id`, `name`, `created`, `creator`, `edited`, `editor`, `pages_visited`, `time_on_site`, `max_trigger_amount`, `trigger_again_after`, `trigger_again_page`, `not_declined`, `not_accepted`, `not_in_chat`, `is_in_chat`, `priority`, `is_active`, `search_phrase`, `save_cookie`, `exclude_mobile`, `exclude_countries`, `data_conditions`) VALUES ('".DBManager::RealEscape($this->Id)."','".DBManager::RealEscape($this->Name)."','".DBManager::RealEscape($this->Created)."','".DBManager::RealEscape($this->Creator)."','".DBManager::RealEscape($this->Edited)."', '".DBManager::RealEscape($this->Editor)."', '".DBManager::RealEscape($this->PagesVisited)."','".DBManager::RealEscape($this->TimeOnSite)."','".DBManager::RealEscape($this->TriggerAmount)."','".DBManager::RealEscape($this->TriggerTime)."',".intval($this->TriggerPage?1:0).",'".DBManager::RealEscape($this->NotDeclined)."', '".DBManager::RealEscape($this->NotAccepted)."',".intval($this->NotInChat ? 1 : 0).",".intval($this->IsInChat ? 1 : 0).",'".DBManager::RealEscape($this->Priority)."', '".DBManager::RealEscape($this->IsActive)."', '".DBManager::RealEscape($this->SearchPhrase)."', '".DBManager::RealEscape(($this->SaveInCookie) ? 1 : 0)."', '".DBManager::RealEscape(($this->ExcludeMobile) ? 1 : 0)."', '".DBManager::RealEscape($this->ExcludeCountries)."', '".DBManager::RealEscape(serialize($this->DataConditions))."');";
	}

	function GetXML()
	{
		$xml = "<ev id=\"".base64_encode($this->Id)."\" sc=\"".base64_encode($this->SaveInCookie)."\" nacc=\"".base64_encode($this->NotAccepted)."\" ndec=\"".base64_encode($this->NotDeclined)."\" name=\"".base64_encode($this->Name)."\" prio=\"".base64_encode($this->Priority)."\" created=\"".base64_encode($this->Created)."\" nic=\"".base64_encode($this->NotInChat)."\" iic=\"".base64_encode($this->IsInChat)."\" creator=\"".base64_encode($this->Creator)."\" editor=\"".base64_encode($this->Editor)."\" edited=\"".base64_encode($this->Edited)."\" tos=\"".base64_encode($this->TimeOnSite)."\" ta=\"".base64_encode($this->TriggerAmount)."\" tt=\"".base64_encode($this->TriggerTime)."\" tp=\"".base64_encode($this->TriggerPage?1:0)."\" pv=\"".base64_encode($this->PagesVisited)."\" ia=\"".base64_encode($this->IsActive)."\" sp=\"".base64_encode($this->SearchPhrase)."\" em=\"".base64_encode($this->ExcludeMobile ? 1 : 0)."\" ec=\"".base64_encode($this->ExcludeCountries)."\">\r\n";
...
class Mailbox
{
    ...
    {
        if($_remove)
            $_text = str_replace("<!--lz_ref_link2-->","",$_text);
        else if($_html)
            $_text = str_replace("<!--lz_ref_link2-->","<br><br>".parseURL(@Server::$Configuration->File["gl_cpae"]),$_text);
        else
            $_text = str_replace("<!--lz_ref_link2-->","\r\n\r\n".@Server::$Configuration->File["gl_cpae"],$_text);
        return $_text;
    }
...
class DataInput
{
    ...
    static function GetChatLoginInputs($_html,$_maxlength,$_overlay=false,$inputshtml="")
    {
        ...
        {
            if(!empty($_GET["grot"]))
                $_html = str_replace("<!--chat_login_group_top-->",IOStruct::GetFile(PATH_TEMPLATES . "overlays/chat/login_group.tpl"),$_html);
            else
                $_html = str_replace("<!--chat_login_group_bottom-->",IOStruct::GetFile(PATH_TEMPLATES . "overlays/chat/login_group.tpl"),$_html);
        }
        else
        {
            if(!empty($_GET["grot"]))
...
class KnowledgeBase
{
    ...
    static function GetMatches($root,$_question,$_language="",$_botOnly=false)
    {
    ...
    static function GetEntries($_root="",$_language="")
    {
        $rlist = array();
    ...
    static function GetExternalURL($_id="",$_absolute=true)
    {
        $params = KnowledgeBase::GetAllowedParameters();
        if(!empty($params))
            $params = "&" . $params;
        if(!empty($_id))
            $params .= "&id=" . $_id;
    ...
    {
        $allowed = array("e"=>true,"el"=>true,"epc"=>true,"etc"=>true,"ckf"=>true);
