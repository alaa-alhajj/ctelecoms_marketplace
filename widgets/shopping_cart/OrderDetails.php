        ...
        <table ID="list" width='100%' class="table table-stripped table-valign-middle table-striped ShoppingCartTable">
            ...
            <tbody>
                ...
                foreach ($order_products_info as $product) { //purchase order details 
                    ...
                    //print_r($product);
                    $qty=1;
                    $pro_Discount_val=$qty*($product_price*$total_discount/100);
                    $price_after_discount=($product_price -($product_price*$total_discount/100));
                    $total_widthDiscount=$qty * $price_after_discount;
                    $total_widthOutDiscount=$qty* $product_price;
                    ...
                    echo "<td>" .number_format($total_widthOutDiscount,2,'.',',') . "$</td>";
                ...
                echo "</tbody>";
                echo "<tfoot>";
                echo "<tr><td colspan='8' ><b>Total discount:</b> $<span class='DiscountOrderCart'>".number_format($allPros_total_discount,2,'.',',')."<span></td></tr>"
                . "<tr><td colspan='8' ><b>Total price before discount:</b> $<span class='TotalPriceBeforeDiscount'>".number_format($total_price_before_discount,2,'.',',')."<span></td></tr>"
